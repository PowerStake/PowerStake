...
unsigned int nStakeMaxAge = 256 * 24 * 60 * 60; // 256 days
unsigned int nModifierInterval = 5 * 60; // 5 minutes time to elapse before new modifier is computed
...
const string strMessageMagic = "PowerStake Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "PowerStake", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Disney to open 'Star Wars' theme parks CNN 22-08-2015 10:31 PM";
        CTransaction txNew;
        txNew.nTime = 1440275518;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1440275518;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1115980;
		if(fTestNet)
        {  
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xb17bde692cd871e691b7a754beced6b30f07bb0afb9625e7f3d42d3ee4714e6f"));
        block.print();
